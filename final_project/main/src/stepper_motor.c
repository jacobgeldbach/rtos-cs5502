...
void drive_stepper_motor(void* pv_parameters)
{
    ...
    /* I want the motor starting at its leftmost part of its rotation angle */
    int count_half_steps = (HALFSTEPS_PER_ROTATION / 3);
    ...
    uint16_t us_delay[4] = { 0, 2500, 1750, 1200 };
    while (1) {
        uint32_t notif;
        ...
        if (run) {
            ...
            if (counter_clockwise == false) {
                if ((count_half_steps + 1) > (HALFSTEPS_PER_ROTATION / 3)) {
                    counter_clockwise = true;
                }
                else {
                    count_half_steps = count_half_steps + 1;
                }

            }
            else {
                if ((count_half_steps - 1) < 0) {
                    counter_clockwise = false;
                }
                else {
                    count_half_steps = count_half_steps - 1;
            ...
            /* Try and take radar mutex and update current_angle field if available, if not move on*/
            if (xSemaphoreTake(radar_mutex, 0) == pdTRUE) {
                /* Calculate the current radar angle
                * This is based off count_half_steps == 0 as degree 0 for the radar and max being HALFSTEPS_PER_ROTATION/3 which is 120 degrees */
                latest_radar.current_angle = ((float)count_half_steps / (HALFSTEPS_PER_ROTATION / 3)) * 120.0f;
            ...
            if (notif != BTN1) {
                ...
                while (count_half_steps != (HALFSTEPS_PER_ROTATION / 3)) {
                    const int i = count_half_steps % 8;
                    gpio_set_level(MOTOR_PIN_A, (halfsteps[i] & PIN_A_MASK) >> 3);
                    gpio_set_level(MOTOR_PIN_B, (halfsteps[i] & PIN_B_MASK) >> 2);
                    gpio_set_level(MOTOR_PIN_C, (halfsteps[i] & PIN_C_MASK) >> 1);
                ...
                /* Return magnetic coil pins back to zero so motor doesn't get super hot */
                gpio_set_level(MOTOR_PIN_A, 0);
                gpio_set_level(MOTOR_PIN_B, 0);
                gpio_set_level(MOTOR_PIN_C, 0);
                gpio_set_level(MOTOR_PIN_D, 0);
